...
<h2>Setup</h2>
The project is originally set up in the following environment.
<ul>
  <li>Windows 11 OS</li>
  <li>Rustrover IDE</li>
...
<ol>
  ...
  <li>Open browser and make sure http://localhost:1024 if responding</li>
  ...
  <li>To conduct a batch prediction, go to test folder, run command "<li>Go to test folder, run command "curl -X POST http://localhost:1024/predict -H "Content-Type: application/json" --data @data.json"</li>"</li>
</ol>
<h2>Take Aways</h2>
...
<br/>
Third, practically speaking, Rust cannot battle with decades of libraries already there for C++/Java/C#/Python, in order to get "financially" sound, tons of wrappers/bridges are adopted everywhere.  In my sample, the load / execution of onnx model file is a C++ wrapper, which apparently leads to some unsafe layer of codes.  Would it be possible to rewrite everything with safe layer?  The consensus from the community is clearly a "no". 
<h3>Why you should (or should not) go Rust</h3>
If dealing with hardware/memory management directly, no safe layer will be possible with the currect technology stands.  Therefore, most likely switch to Rust will not buy you memory safe, but if there are other reasons for switch then it maybe benifical.
<br/>
If dealing with business logic, the benefits for getting of rid GC style of memory management for Java / C# is very marginal.  The focus here is to get business logic correct and fits into complicated context for daily operations.
<br/>
If dealing with realtime system with no direct hardware access, that should be the sweet spot, so most efforts now are inside cloud vendors, database engines etc.  In those systems, performance and relibility is a critical piece, and they do not always need touch hardware directly.
<h3>Should you consider hybrid approach</h3>
Given the ecosystem size for other languages, most proposed approach is hybrid / combined Rust with existing stuff.  However, some serious considerations should take in first before the jump.
<br/>
Rust is designed from a very different paradigm -- it does not have class/inheritance, it enforces memory ownership, it handles parallel differently -- all of those pose some (or a lot) design conflicts among the team.  That can be very clearly see from the debate of introduction of Rust in Linux.
<br/>
A more practical question is where to draw the line -- only use Rust for new things / rewrite partial old system / complete overhaul -- the decision will take a very long time to come if truely based on analysis; most nowadays, we are seeing marketing slogans rather than real analysis results.
<br/>
As common sense, start with baby steps, and then if there are real/tangible benefits, pour in more resources.  One thing to point out, even for experienced developers, it will be considerable amount of time before they can be proficient in Rust, due to the deifferent design approach.
